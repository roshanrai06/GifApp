package com.roshan.dev.gifapp.domain.utils;
...
public class AnimatedGIFWriter {
    ...
    private void write(int[] pixels, int imageWidth, int imageHeight, OutputStream os) throws Exception {
        ...
        writeFrame(pixels, imageWidth, imageHeight, 0, os);
    ...
    public void writeAnimatedGIF(Bitmap[] images, int[] delays, OutputStream os) throws Exception {
        ...
        for (int i = 0; i < images.length; i++) {
            ...
            writeFrame(pixels, imageWidth, imageHeight, delays[i], os);
        }
    ...
    public void writeFrame(OutputStream os, Bitmap frame, int delay) throws Exception {
        ...
        writeFrame(pixels, imageWidth, imageHeight, delay, os);
    }
    ...
    private void writeFrame(int[] pixels, int imageWidth, int imageHeight, int delay, OutputStream os) throws Exception {
        writeFrame(pixels, imageWidth, imageHeight, 0, 0, delay, GIFFrame.DISPOSAL_RESTORE_TO_BACKGROUND, GIFFrame.USER_INPUT_NONE, os);
    }
    ...
    private static class WuQuant {
        ...
        private static final int QUANT_SIZE = 33;// quant size
        ...
        private final float[][][] m2 = new float[QUANT_SIZE][QUANT_SIZE][QUANT_SIZE];
        private final long[][][] wt = new long[QUANT_SIZE][QUANT_SIZE][QUANT_SIZE];
        private final long[][][] mr = new long[QUANT_SIZE][QUANT_SIZE][QUANT_SIZE];
        private final long[][][] mg = new long[QUANT_SIZE][QUANT_SIZE][QUANT_SIZE];
        private final long[][][] mb = new long[QUANT_SIZE][QUANT_SIZE][QUANT_SIZE];
    ...
    private static class InverseColorMap {
        private final int bitsReserved;// Number of bits used in color quantization.
        private final int bitsDiscarded;// Number of discarded bits
        private final int maxColorVal;// Maximum value for each quantized color
        private final int invMapLen;// Length of the inverse color map
        // The inverse color map itself
        private final byte[] invColorMap;
